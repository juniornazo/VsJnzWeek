...
class PlayState extends MusicBeatState
{
    ...
    public static var StreetSongs:Array<String>;
	public static var MadStreetSongs:Array<String>;
	...
	var dialogue:Array<String> = [':bf:strange code', ':bfmiss:', ':gf:', ':jnz:', ':jnzangry:', ':jnzred:', ':jnzbroke:', ':jnzpissed:', ':dad:>:'];
	...
	override public function create()
	{
        ...
        StreetSongs = ["shine-stars", "fasting-time"];
		MadStreetSongs = ["mad-buns"];
		...
		if (Config.portuguese){

		if(FileSystem.exists("assets/data/" + SONG.song.toLowerCase() + "/" + SONG.song.toLowerCase() + "Dialogue-PTBR.txt")){
			try{
				dialogue = CoolUtil.coolTextFile("assets/data/" + SONG.song.toLowerCase() + "/" + SONG.song.toLowerCase() + "Dialogue-PTBR.txt");
			}
			catch(e){}
		}
	}
		...
		if (SONG.stage == null) {
            ...
            else if(StreetSongs.contains(SONG.song.toLowerCase())) { stageCheck = 'Street'; }
			else if(MadStreetSongs.contains(SONG.song.toLowerCase())) { stageCheck = 'MadStreet'; }
        ...
        if (stageCheck == 'Street')
            {
            curStage = "Street";
            var bg:FlxSprite = new FlxSprite(-270, -300).loadGraphic(Paths.image('street/streetcity_bg'));
            bg.scrollFactor.set(0.9, 0.9);
            bg.antialiasing = true;
            bg.setGraphicSize(Std.int(0.5 * bg.width));
            bg.updateHitbox();
            add(bg);
            ...
            else if (stageCheck == 'MadStreet')
            {
            curStage = "MadStreet";
            var bg:FlxSprite = new FlxSprite(-270, -300).loadGraphic(Paths.image('street/madbuns_street'));
            bg.scrollFactor.set(0.9, 0.9);
            bg.antialiasing = true;
            bg.setGraphicSize(Std.int(0.5 * bg.width));
            bg.updateHitbox();
	...
	/*function videomadbun():Void {
		inCutscene = true;
		FlxG.switchState(new VideoState('assets/videos/MadBuns.webm', new PlayState(), 169, true));	
	}*/
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer)
		{
			...
			else
			{
				/*if (SONG.song.toLowerCase() == 'mad-buns')
					{
						new FlxTimer().start(0, function(swagTimer:FlxTimer)
							{
						inCutscene = true;
						add(dialogueBox);
						add(red);
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					...
					case 'MadStreet':
					babyArrow.frames = FlxAtlasFrames.fromSparrow('assets/images/street/MadBunnyNOTE_assets.png', 'assets/images/street/MadBunnyNOTE_assets.xml');
						babyArrow.animation.addByPrefix('green', 'arrowUP');
						babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
						babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
						babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.THREE)
			{
				if (iconP1.animation.curAnim.name == 'bf-alpha')
					iconP1.animation.play(SONG.player1);
				else
					iconP1.animation.play('bf-alpha');
			}

			if (FlxG.keys.justPressed.FOUR)
				{
					if (iconP2.animation.curAnim.name == 'face')
	...
	override function beatHit()
	{
        ...
        if (curSong.toLowerCase() == 'shine-stars' && curStep >= -1 && curStep <= 10)
            {
                dad.playAnim('count', false);
    
            }
            
            if (curSong.toLowerCase() == 'shine-stars' && curBeat >= 105 && curBeat <= 107 && camZooming && FlxG.camera.zoom < 1.35)
            {
                startedMoving = true;
		...
		switch (curStage)
		{
			...
			case "mall":
                ...
                case "Street", "MadStreet":
                    friends.animation.play('idle', true);
